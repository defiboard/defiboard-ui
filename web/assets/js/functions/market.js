    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        if (divMarketListCrypto.childElementCount >= i + 1) {
          let current =
            divMarketListCrypto.getElementsByClassName("market-list-row")[i];
          if (current?.innerHTML !== rows[i].innerHTML) {
            ...
            if (currentIcon !== rows[i].getElementsByClassName("icon")[0]) {
              currentIcon.setAttribute(
                "src",
                rows[i].getElementsByClassName("icon")[0].getAttribute("src")
              );
            }

            if (
              currentInfo.innerHTML !==
              rows[i].getElementsByClassName("info-wrapper")[0].innerHTML
            ) {
              currentInfo.innerHTML =
                rows[i].getElementsByClassName("info-wrapper")[0].innerHTML;
            }
...
async function populateMarketListStocks(page, currency) {
  try {
    ...
    let marketStocksData = !empty(watchlistStockSymbols)
      ? await fetchStockPrice(currency, watchlistStockSymbols, false)
      : {};
    ...
    let rows = await createWatchlistListRows(
      {},
      marketStocksData,
      watchlistData
    );
    ...
    if (
      divMarketListStocks.getElementsByClassName("loading-icon").length > 0 ||
      divMarketListStocks.childElementCount !== rows.length
    ) {
    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        if (divMarketListStocks.childElementCount >= i + 1) {
          let current =
            divMarketListStocks.getElementsByClassName("watchlist-list-row")[i];
          if (current?.innerHTML !== rows[i].innerHTML) {
            let currentInfo = current.getElementsByClassName("info-wrapper")[0];
...
function createMarketListCryptoRows(marketData, page, currency) {
  ...
  for (let i = 0; i < 100; i++) {
    try {
      ...
      let priceChangeDay = formatPercentage(
      ...
      //   const x = function testFunc() {
      //     document.getElementById("buy").innerHTML = alert("working");
      //   };
      ...
      div.innerHTML = `
				...
				<div class="info-wrapper audible-pop">
					...
					<button id="buy" style="background-color: #2FCD76; /* Green */
												border: none;
												color: white;
												padding: 5px 15px;
												text-align: center;
                ...
                currencySymbols[currency] + separateThousands(price)
              }</span>
							<span class="ath">ATH: ${
                currencySymbols[currency] + separateThousands(ath)
              }</span>
							<span class="high-24h">24h High: ${
                currencySymbols[currency] + separateThousands(high24h)
              }</span>
							<span class="low-24h">24h Low: ${
                currencySymbols[currency] + separateThousands(low24h)
              }</span>
							<span class="volume">Volume: ${
                currencySymbols[currency] + abbreviateNumber(volume, 2)
              }</span>
						</div>
						<div class="bottom audible-pop">
							<span class="market-cap">Market Cap: ${
                currencySymbols[currency] + separateThousands(marketCap)
              }</span>
          ...
          div.innerHTML = `
						...
						<div id="myModal" class="modal">
								...
								<div class="modal-content">
									...
									<iframe height="600" title="Transak On/Off Ramp Widget (Website)" src="" allowtransparency="true" allowfullscreen="" style="display: block; width: 100%; max-height: 550px; max-width: 450px; border: none;"></iframe>
								</div>
...
function addMarketCryptoData(previousElement, info) {
  ...
  div.innerHTML = `
		<div class="info-container margin-bottom">
			...
			<span class="price">Price: ${
        currencySymbols[info.currency] + separateThousands(info.price)
      }</span>
			<span class="ath">ATH: ${
        currencySymbols[info.currency] + separateThousands(info.ath)
      }</span>
			<span class="high-24h">24h High: ${
        currencySymbols[info.currency] + separateThousands(info.high24h)
      }</span>
			<span class="low-24h">24h Low: ${
        currencySymbols[info.currency] + separateThousands(info.low24h)
      }</span>
			<span class="volume">Volume: ${
        currencySymbols[info.currency] + abbreviateNumber(info.volume, 2)
      }</span>
			<span class="market-cap">Market Cap: ${
        currencySymbols[info.currency] + separateThousands(info.marketCap)
      }</span>
...
function addMarketStockData(previousElement, info) {
  ...
  div.innerHTML = `
        ...
        currencySymbols[info.currency] + separateThousands(info.price)
			...
			<span class="high-1y">1Y High: ${
        currencySymbols[info.currency] + separateThousands(info.high1y)
      }</span>
			<span class="low-1y">1Y Low: ${
        currencySymbols[info.currency] + separateThousands(info.low1y)
      }</span>
			<span class="market-cap">Market Cap: ${
        currencySymbols[info.currency] + separateThousands(info.marketCap)
      }</span>
...
async function showCryptoMarketData(info) {
  try {
    ...
    let urltransak =
      "https://staging-global.transak.com?apiKey=b42f80c6-09e3-4995-a781-660fc5a2c8f4&defaultCryptoCurrency=" +
    ...
    let popup = new Popup(
      ...
      `<div style="display:flex">
		<div  style="width:30%">
			<iframe height="450" title="Transak On/Off Ramp Widget (Website)" src=${urltransak} allowtransparency="true" allowfullscreen="" style="display: block; width: 100%;  max-width: 450px; border: none; border-radius: 5px;"></iframe>
		</div>
	  	<div style="width:70%">
		  	<div class="chart-wrapper"></div>
		</div>
	 </div>
		<div><div  style="width:100%"><div class="marketdata-show"></div><span>${
      empty(data?.description?.en)
        ? "No description found."
        : data?.description?.en
    }</span></div></div>`,
      { cancelText: "Dismiss", confirmText: "-", page: "market" }
    );
    ...
    try {
      ...
      if (validJSON(historicalData)) {
        ...
        // let divTrancat = document.createElement("div");
        // divTrancat.setAttribute("width", "50%");
        // divTrancat.innerHTML = `<iframe height="700" title="Transak On/Off Ramp Widget (Website)" src="${urltransak}" allowtransparency="true" allowfullscreen="" style="display: block; width: 100%;  max-width: 450px; border: none;"></iframe>`;

        // divChart.parentNode.insertBefore(divTrancat, divChart.nextSibling);
        let divMarketdata =
          popup.element.getElementsByClassName("marketdata-show")[0];

        addMarketCryptoData(divMarketdata, info);
      } else {
...
function showStockMarketData(infoPrice, infoHistorical) {
  try {
    ...
    let popup = new Popup(
      "full",
      "full",
      `${infoHistorical.meta.symbol} - Market Data`,
      `<div class="chart-wrapper"></div>`,
      { cancelText: "Dismiss", confirmText: "-", page: "market" }
    );
    ...
    try {
      ...
      generateChart(
        divChart,
        `${infoHistorical.meta.symbol} Price`,
        parsed.labels,
        parsed.tooltips,
        infoHistorical.currency,
        parsed.prices,
        colors
      );
    ...
    errorNotification(
      `Couldn't fetch market data for ${infoHistorical.meta.symbol}`
    );
    console.log(error);
  }
}
...
async function showMarketSearchResult(
  popup,
  inputSearch,
  symbol,
  currency,
