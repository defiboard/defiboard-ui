...
function showSideMenu() {
  ...
  divSideMenuWrapper.style.right = `-${divSideMenuWrapper.scrollWidth}px`;

  let right = divSideMenuWrapper.scrollWidth * -1;

  let animation = setInterval(() => {
    divSideMenuWrapper.style.right = `${right}px`;
...
function hideSideMenu() {
  ...
  let animation = setInterval(() => {
    ...
    if (right <= divSideMenuWrapper.scrollWidth * -1) {
      clearInterval(animation);
...
function setPage(page) {
  page = empty(page)
    ? defaultChoices.defaultPage.toLowerCase()
  ...
  switch (page) {
    case "chatbot":
      ...
      break;
    case "pools":
      populatePools(1, "uniswap_v2");
      firstFetch.pools = false;
      break;
...
async function checkBackdrop() {
  ...
  if (
    "assetIconBackdrop" in choices &&
    choices.assetIconBackdrop === "enabled"
  ) {
...
async function checkHoldingsOnDashboard() {
  ...
  if (
    "holdingsOnDashboard" in choices &&
    choices.holdingsOnDashboard === "enabled"
  ) {
...
function showAssetMatches(referenceNode, list, marginBottom) {
  if ("matches" in list && Object.keys(list.matches).length > 1) {
    ...
    errorNotification(
      "Two or more assets have the same symbol. Please choose an asset from the list."
    );
...
function addTooltips() {
  ...
  tippy(buttonDashboardBudgetEdit, {
    content: "Edit Budget",
    placement: "bottom",
  });
  tippy(buttonDashboardWatchlistAdd, {
    content: "Add Asset",
    placement: "bottom",
  });
  ...
  tippy(".holdings-card.value", {
    content: "Total Portfolio Value",
    placement: "right",
  });
  tippy(buttonActivityHelp, { content: "Help", placement: "top" });
  tippy(buttonSettingsLogoutEverywhere, {
    content:
      "Deletes all your active session tokens, causing you to get logged out on every device.",
    placement: "right",
  });
  tippy(buttonSettingsUserRegistration, {
    content:
      "Enables or disables the ability for users to register new accounts.",
    placement: "right",
  });
  tippy(buttonSettingsStockAPIType, {
    content:
      "Switches between the internal stock API, and an external one. The internal one would not have API rate limits if used in moderation by a handful of users, but isn't recommended if user registration is enabled. The external one allows each user to set an API key, and make 100 requests per day. Market data is aggressively cached and shared between users to limit API requests to a minimum, and stock prices only update once a day. Both APIs are unofficial, and could cease to work at any point.",
    placement: "bottom",
  });
}
...
function showLoading(limit, text = "") {
  ...
  element.innerHTML =
    '<div class="loading-icon"><div></div><div></div></div><span id="loading-text">' +
    text +
    "</span>";
...
function hideLoading() {
  for (
    let i = 0;
    i < document.getElementsByClassName("loading-screen").length;
    i++
  ) {
...
function audibleElement(element) {
  try {
    ...
    for (let i = 0; i < popType.length; i++) {
      if (
        element.classList.contains(popType[i]) ||
        element.parentElement.classList.contains(popType[i])
      ) {
    ...
    for (let i = 0; i < switchType.length; i++) {
      if (
        element.classList.contains(switchType[i]) ||
        element.parentElement.classList.contains(switchType[i])
      ) {
    ...
    for (let i = 0; i < tags.length; i++) {
      if (
        element.tagName.toLowerCase() === tags[i] ||
        element.parentElement.tagName.toLowerCase() === tags[i]
      ) {
        return { audible: true, type: "pop" };
      }
    }
