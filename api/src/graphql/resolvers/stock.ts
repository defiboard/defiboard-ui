...
// The DefiBoard API uses two third-party APIs for stock market data. The first one is the Yahoo Finance API directly, the second is an API that also uses the Yahoo Finance API, but requires an API key to work. This is required because if DefiBoard is used by multiple people, then its API would be making potentially hundreds of requests to the Yahoo Finance API directly, which might result in rate limits for all users. The admin can switch the API to the external one through the web app or desktop app's settings page, at which point each user would require an API key from the external API provider to access stock market functionality.
let stockAPITypes = {
...
export async function readStockHistorical({
  token,
  userID,
  keyAPI,
  assetSymbol,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (valid) {
        ...
        db.db?.get(
          "SELECT * FROM Stock WHERE assetSymbol = ?",
          [assetSymbol],
          async (error, row) => {
            if (error) {
              console.log(error);
              ...
              if (
                row === undefined ||
                !Utils.validJSON(row.historicalData) ||
                Utils.refetchRequired(JSON.parse(row.historicalData).time)
              ) {
                try {
                  let historicalData: any = await getHistoricalData(
                    assetSymbol,
                    keyAPI
                  );

                  if (
                    Utils.validJSON(historicalData) &&
                    "error" in JSON.parse(historicalData)
                  ) {
                  ...
                  db.runQuery(
                    "INSERT OR REPLACE INTO Stock (assetSymbol, historicalData, priceData) VALUES (?, ?, ?)",
                    [assetSymbol, historicalData, row?.priceData || ""]
                  );
                      ...
                      if (row !== undefined) {
                        db.runQuery(
                          ...
                          [
                            ...
                            JSON.stringify({
                              time: priceData.time,
                              priceData: priceData.priceData[symbol],
                            }),
                          ]
                        );
                      } else {
        ...
        db.db?.get(
          ...
          async (error, row) => {
            if (error) {
              ...
              if (
                row === undefined ||
                !Utils.validJSON(row.priceData) ||
                Utils.refetchRequired(JSON.parse(row.priceData).time)
              ) {
...
async function getHistoricalData(assetSymbol: string, keyAPI: string) {
  ...
  let stockAPI =
    settings.stockAPIType === "internal"
      ? stockAPITypes.internal
      : stockAPITypes.external;

  let now = Math.floor(new Date().getTime() / 1000);

  let historicalData: any = await Utils.request(
    "GET",
    stockAPI +
      "/v8/finance/chart/" +
      assetSymbol.toUpperCase() +
      "?range=1y&interval=1d&lang=en",
    null,
    [["X-API-KEY", keyAPI]]
  );
  ...
  if ("chart" in historicalData) {
    ...
    "message" in historicalData &&
    historicalData.message === "Limit Exceeded"
  ) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < symbols.length; i++) {
        ...
        let priceData: any = await Utils.request(
          "GET",
          stockAPI + "/v6/finance/quote?lang=en&symbols=" + joinedSymbols,
          null,
          [["X-API-KEY", keyAPI]]
        );

        if (
          "quoteResponse" in priceData &&
          "result" in priceData.quoteResponse
        ) {
          ...
          "message" in priceData &&
          priceData.message === "Limit Exceeded"
        ) {
